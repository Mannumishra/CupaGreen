...
const AllCategory = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/get-category'); // Update the endpoint as per your backend
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete.isConfirmed) {
            try {
                await axios.delete(`http://localhost:8000/api/delete-category/${id}`); // Update the endpoint as per your backend
